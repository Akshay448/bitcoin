...
```bash
btc-apis$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
btc-apis$ sam local start-api
btc-apis$ curl http://localhost:3000/
```
...
```bash
btc-apis$ sam logs -n HelloWorldFunction --stack-name btc-apis --tail
```
...
```bash
aws cloudformation delete-stack --stack-name btc-apis
```

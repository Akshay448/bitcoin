...
```bash
blockchain-apis$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
blockchain-apis$ sam local start-api
blockchain-apis$ curl http://localhost:3000/
```
...
```bash
blockchain-apis$ sam logs -n HelloWorldFunction --stack-name blockchain-apis --tail
```
...
```bash
blockchain-apis$ cd bitcoin
bitcoin$ npm install
bitcoin$ npm run test
```
...
```bash
aws cloudformation delete-stack --stack-name blockchain-apis
```
